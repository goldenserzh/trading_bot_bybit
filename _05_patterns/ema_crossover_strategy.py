...
def detect_ema_crossover_signals(df, short_period=9, long_period=21):
    ...
    df = calculate_ema(df, short_period) #копирует данные
    df = calculate_ema(df, long_period) #копирует данные
...
if __name__ == "__main__":
    ...
    signals = df[df['signal_ema'] != 0]
    print(signals[['timestamp', 'close', 'EMA_9', 'EMA_21', 'signal_ema']])
    print(df.isna().sum())
